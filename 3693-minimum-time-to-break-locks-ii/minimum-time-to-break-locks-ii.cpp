...
public:
    int findMinimumTime(vector<int>& strength) {
        ...
        for (int i = 0; i < n; ++i) {
            for (int j = 0; j < n; ++j) {
                // Initialize the weight matrix \( c_{ij} = \lceil \frac{s_j}{i + 1} \rceil \) where \( s_j \) is the strength of lock \( j \).
        ...
        for (int sword = 0; sword < n; ++sword) {
            ...
            while (true) {
                ...
                int delta = numeric_limits<int>::max(), next_lock = -1; // Initialize \( \delta = \infty \) and next lock.
                ...
                // Step 2: Update dual variables to adjust for the smallest slack
                for (int i = 0; i < n; ++i) {
                    // Update dual variables based on visited status.
                    if (visited_sword[i]) y_sword[i] += delta; // \( y_v \leftarrow y_v + \delta \) for visited swords.
                    if (visited_lock[i]) y_lock[i] -= delta; // \( y_w \leftarrow y_w - \delta \) for visited locks.
                    else min_slack[i] -= delta; // \( \delta_w \leftarrow \delta_w - \delta \) for non-visited locks.
                }
        ...
        for (int i = 0; i < n; ++i) {
            ...
            total_time += weight[i][matched_lock[i]]; // Total time is the sum of costs \( c_{vw} \) for matched pairs.
        }
