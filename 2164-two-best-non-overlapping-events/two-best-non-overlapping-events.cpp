...
vector<int> generateMaxValues(const vector<vector<int>>& events) {
    ...
    int maxTwoEvents(vector<vector<int>>& events) {
        ...
        vector<int> sortedFirstIndices;
        for (const auto& event : events) {
            sortedFirstIndices.push_back(event[0]);
        }
        ...
        for (int i = 0; i < n - 1; i++) {
            ...
            // Find the next valid event using find_if
            int k = events[i][1];
            if(k>=*(sortedFirstIndices.end() - 1)){
                continue;
            }
            auto it = find_if(sortedFirstIndices.begin() + i + 1, sortedFirstIndices.end(), [k](int x) {
                return x > k;
            });
