...
public:
    long long countOfSubstrings(string word, int k) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            // If lo reaches it_high, then no candidate j in the candidate set satisfies vowels condition.
            if (lo == it_high)
                continue;
            // Every candidate corresponding to an iterator in [lo, it_high) yields a valid substring.
            ans += (it_high - lo);
        }
